...
def convert_to_UltraAlignedSegment(header, sequence, quality,
                              barcode_sequence, umi_sequence, header_fw):
    """
    This function converts the input variables 
    (header,sequence,quality,barcode_sequence,umi_sequence)
    to a unaligned pysam.AlignedSegment with the umi and barcode 
    ...
    coord_1 = header_fw.split()[-2].split(":")[-1]
    coord_2 = header_fw.split()[-1].split(":")[-1]
    ...
    # Set the tags
    #aligned_segment.set_tag('uT', )
    aligned_segment.set_tag('B0', barcode_sequence)
    aligned_segment.set_tag('B3', umi_sequence)
    aligned_segment.set_tag('RG', '0')
    aligned_segment.set_tag('B0', barcode_sequence)
    aligned_segment.set_tag('B1', coord_1)
    aligned_segment.set_tag('B2', coord_2)
    
    if barcode_sequence == "TTGCTCGAATAAATAATT":
        print(header_fw)
